...
public class JpaMessageStore implements MessageStore {
    ...
    public int countByChannel(UUID channelId) {
        return (int) Message.count("channelId = ?1 AND tenancyId = ?2", channelId, currentPrincipal.tenancyId());
    }
    ...
    public Optional<Message> findLastMessage(final UUID channelId) {
        return Message.<Message>find("channelId = ?1 AND tenancyId = ?2 ORDER BY id DESC",
