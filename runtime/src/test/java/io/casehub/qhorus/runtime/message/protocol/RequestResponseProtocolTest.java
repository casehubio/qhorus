...
class RequestResponseProtocolTest {
    ...
    void openQueriesAtThreshold_advisoryOnNewQuery() {
        ...
        var advisories = protocol.evaluate(ctx(MessageType.QUERY, "a", commitments));
        assertThat(advisories).hasSize(1);
        assertThat(advisories.get(0).source()).isEqualTo("REQUEST_RESPONSE");
        assertThat(advisories.get(0).severity()).isEqualTo(Severity.WARNING);
        assertThat(advisories.get(0).message()).contains("3 unanswered QUERYs");
        assertThat(advisories.get(0).evidence()).containsEntry("openQueryCount", 3);
        assertThat(advisories.get(0).evidence()).containsEntry("threshold", 3);
    }

    @Test
    void nonResponseWithOpenQueries_advisory() {
        List<Commitment> commitments = List.of(openQuery("q1", "a"));
        var              advisories  = protocol.evaluate(ctx(MessageType.STATUS, "b", commitments));
        assertThat(advisories).hasSize(1);
        assertThat(advisories.get(0).source()).isEqualTo("REQUEST_RESPONSE");
        assertThat(advisories.get(0).severity()).isEqualTo(Severity.ADVISORY);
        assertThat(advisories.get(0).message()).contains("open QUERYs awaiting RESPONSE");
    }
