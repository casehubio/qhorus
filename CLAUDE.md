...
- `@TestTransaction` also isolates test data from `@Scheduled` beans — the scheduler runs on a separate thread with its own transaction and cannot see uncommitted test data. Tests that call service methods (like `watchdogService.evaluateAll()`) directly AND have a `@Scheduled` driver active must use `@TestTransaction` to prevent the scheduler from picking up in-flight test state and firing spurious side effects.
- Panache static queries (`Entity.<Entity>find()`, `Entity.listAll()`, etc.) bypass CDI `@Alternative @Priority(1)` store implementations — they always query JPA directly. With InMemory stores active via `quarkus.arc.selected-alternatives`, the JPA entity tables are empty. Use the store/service layer (`tools.listChannels()`, `channelStore.findByName()`) to query domain entities in tests that use InMemory stores; never use Panache statics in that context.
