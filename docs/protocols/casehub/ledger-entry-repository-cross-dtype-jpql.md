...
type: rule
scope: repo
applies_to: "LedgerEntryJpaRepository, any future LedgerEntryRepository implementation in casehub-qhorus"
severity: critical
...
Any class implementing `LedgerEntryRepository` must use
...
before dispatching a qhorus message. Qhorus-specific repositories (`MessageLedgerEntryRepository`) that do NOT implement the interface may use subtypes
